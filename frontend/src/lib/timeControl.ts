 ...
 * Times are floored to 1-minute buckets so identical URLs hit the nginx tile
 * cache; `null` means live mode (the route query carries no `t`).
 *
 * The bucket is the map's lag behind reality — every viewer asking for the same
 * bucket shares one render, so this trades freshness for herd protection, not
 * for per-tile cost (an uncached world tile is ~20ms). A minute keeps the map
 * within a page-refresh of the list, which queries the exact instant. Keep
 * nginx's proxy_cache_valid for /tiles/ in step with it. */
 ...
 * list was correct. Bucketing means the URL rotates every minute, so each
...
/** Call `onRoll` when the bucket rolls over, so a live map left open
